...
public class PanelAnim : HCMonoBehaviour
{
    ...
    [ShowIf(nameof(UsePopupAnim))]
    public bool animInFade = true;
    ...
    private Vector3 PopupLocalPos => _target.root.transform.InverseTransformPoint(popup.position);
        ...
        switch (animIn)
        {
            case AnimInType.FromTop:
                if (UsePopupAnim && popup)
                {
                    ...
                    float offset = rootRect.height / 2 - PopupLocalPos.y + popupRect.height / 2;
            ...
            case AnimInType.FromLeft:
                if (UsePopupAnim && popup)
                {
                    ...
                    float offset = rootRect.width / 2 + PopupLocalPos.x + popupRect.width / 2;
